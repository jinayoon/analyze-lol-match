...
**Support quest item evolutions (Vigilant Wardstone / Zaz’Zak’s Realmspike / etc.):** These items are obtained by completing the support quest, not by purchasing outright. Do NOT critique the player for “choosing” a quest evolution item as their first purchase — they didn’t choose to spend gold on it. The only valid critique here is which evolution they picked, if it mismatches their kit (e.g., picking Dreamcatcher on a champion with no heals/shields to proc its passive).
