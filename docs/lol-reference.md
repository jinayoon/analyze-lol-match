...
In-repo reference for **game domain** (ranked, queues, objectives, runes, terminology) and **Riot Games API** usage (endpoints, routing, rate limits, Data Dragon). Use this file from skills or agents with **Read** instead of depending on a separate `lol-helper` skill.

**This repo:** match export is implemented by **`scripts/fetch_lol_match.py`** (`RIOT_API_KEY`, ranked Summoner's Rift defaults, regional routing, 429 handling). The optional **Twisted** section below applies only if you add a Node/TypeScript client.
...
## Match fetch in this repository

Prefer **`scripts/fetch_lol_match.py`** for match + timeline JSON (see script `--help`). It aligns with this document’s **platform vs regional** routing and ranked queue IDs **420 / 440**. By default it also downloads Data Dragon **`items.json`** and **`champion.json`** into the same output directory (use `--no-ddragon` to skip). Coaching scripts read those paths instead of `/tmp/lol_items.json`.

---

## Twisted library (optional — Node / TypeScript)

If you add a JS/TS stack, the **`twisted`** npm package is a common Riot API wrapper (not required for the Python fetch script).
...
# Part 3: Itemization Reference

---

## Grievous Wounds (Anti-Heal)

**Mechanics:** GW reduces all healing received by the target by 40% (or 60% at higher stacks on some items). It applies on **any damaging hit** – a single auto-attack, a low-damage poke ability, or a DoT tick – regardless of how much damage the hit deals. A Lulu E shield that deals 50 damage still applies GW. A Soraka W heals nothing if her target has GW active.

**When to buy anti-heal:** Buy a GW item when the enemy team has at least one champion with significant healing or sustain. Champions that reliably justify an anti-heal purchase include:

| Category | Champions |
|---|---|
| High-sustain tanks | Dr. Mundo, Sett, Warwick, Nasus, Volibear, Olaf, Garen |
| Healing carries | Vladimir, Aatrox, Sylas, Soraka, Yuumi, Sona, Swain |
| Lifesteal-dependent | Any champion stacking lifesteal items (BotRK, Ravenous Hydra, Immortal Shieldbow) |
| Rune-dependent healing | Conqueror + extended fights on bruisers |

If any of these are on the enemy team and are actively relevant (not feeding, actually fighting), an anti-heal purchase is justified regardless of who buys it.

**Common anti-heal items by role:**
| Item | Who buys it | Notes |
|---|---|---|
| Oblivion Orb | Mages, mage-supports | Standalone anti-heal, builds into Morellonomicon |
| Morellonomicon | Mages | Full AP item + GW |
| Executioner's Calling | ADC | Cheap early GW component |
| Mortal Reminder | ADC | Full crit item + GW |
| Chempunk Chainsword | Fighters, bruisers | AD + GW |
| Bramble Vest | Tanks | Armor + GW on being hit |
| Thornmail | Tanks | Full armor item + GW |

---

## Support Item Archetypes

When evaluating support item choices, first identify the support's archetype – each has a distinct priority order.

### Enchanter (Lulu, Janna, Soraka, Karma, Nami, Yuumi, Sona, Seraphine)
Win condition: keep the carry alive and amplify their output.

**Standard priority:**
1. **Ardent Censer** – if ADC benefits from attack speed (hypercarries: Jinx, Ashe, Kog'Maw, Twitch, Kaisa)
2. **Shurelya's Battlesong** – chase/disengage active; best when fights are dynamic and mobile
3. **Moonstone Renewer** – sustained healing in prolonged fights; best against poke/attrition
4. **Staff of Flowing Water** – if team has AP carries who benefit from AP amp
5. **Redemption** – AoE heal in teamfights; strong against AOE poke (Brand, Rumble)
6. **Mikael's Blessing** – CC removal; buy when enemy has hard CC that targets your carry (Ashe R, Lissandra R, Leona, Morgana)
7. **Locket of the Iron Solari** – team-wide shield; against AoE burst (Orianna, Amumu, Jinx R)

**Situational deviations:**
- Enemy has heavy healing (DrMundo, Soraka, Aatrox): Oblivion Orb is a valid rush purchase even on low-damage enchanters – GW application comes from any poke hit
- Enemy has assassin diver targeting your carry: Mikael's or Locket earlier
- Team is losing every teamfight on cooldown: Redemption + Locket provide zone healing/shielding that doesn't require positioning

### Tank/Engage Support (Leona, Nautilus, Blitzcrank, Thresh, Alistar, Rakan)
Win condition: land engage, peel after, stay alive through burst.

**Standard priority:**
1. Locket of the Iron Solari or Zeke's Convergence
2. Knight's Vow (if protecting a specific carry)
3. Gargoyle Stoneplate (in sustained teamfights)
4. Warmog's Armor (for HP-threshold items and regen between fights)

### Poke/Mage Support (Lux, Zyra, Xerath, Brand, Vel'Koz)
Win condition: zone, poke to 50% HP, then all-in.

**Standard priority:**
1. Luden's Echo or Shadowflame (burst amp)
2. Sorcerer's Shoes
3. Morellonomicon (if enemy has healing)
4. Zhonya's Hourglass (against AD assassins)
